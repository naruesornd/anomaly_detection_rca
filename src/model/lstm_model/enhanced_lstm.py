...
#     # X comes from dp.df
#     X_all = dp.df[selected_features]

#     # y comes from dp.outputs_df
#     y_all = dp.outputs_df[target_col]

#     # merge into one dataframe for convenience
...
#     train_data_export = pd.DataFrame(np.concatenate([X_train, y_train], axis=1), columns=selected_features + [target_col])
#     val_data_export = pd.DataFrame(np.concatenate([X_val, y_val], axis=1), columns=selected_features + [target_col])
#     test_data_export = pd.DataFrame(np.concatenate([X_test_raw, y_test_raw], axis=1), columns=selected_features + [target_col])
...
#     joblib.dump(scaler_x, scaler_x_path)  # 或者用 pickle.dump()
...
#     joblib.dump(scaler_y, scaler_y_path)  # 或者用 pickle.dump()
...
#     # flatten and remove any NaNs just in case
...
def model_analysis(test_loader, model_weights_path, scaler_x, scaler_y, target_col, device, feature_name):
    ...
    # Transform from "Z-Score" back to "Real Units" (e.g., Bar/PSI)
    # We must reshape to (-1, 1) because scaler expects a 2D column vector
    ...
    # 3. Calculate Metrics (Now in Real Units!)
    assert len(y_pred) == len(y_true), f"Length mismatch"
    ...
    # 添加标注
    plt.title(f'True vs Predicted Values\nR2: {r2:.4f} | MAE: {mae:.4f} for {feature_name}')
    plt.xlabel('Time Steps')
    plt.ylabel('Target Value')
